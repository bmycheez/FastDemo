...
auto myTime1 = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(startTime1 - startTime5).count();
...
auto myTime2 = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(startTime2 - startTime5).count();
auto endTime2 = std::chrono::system_clock::now();
auto latency2 = std::chrono::duration_cast<std::chrono::milliseconds>(endTime2 - startTime2);
#pragma endregion
...
void QCapture(TSQueue<void**>& Q1)
{
    cv::Mat hwcFrame;
    int copied = 0;
    // void** bufferPointer = allocateBuffer(engine, true, true);
    // std::vector<void*> gpuInputBuffers(bufferPointer, bufferPointer + config.frame_cnt_);
    while(i > 0)
    {
	    ...
	    Q1.push(gpuInputBuffers.data());
    }
    // for (size_t j = 0; j < gpuInputBuffers.size(); ++j)
    //     cudaFree(gpuInputBuffers[j]);
    return;
}

void QModel(TSQueue<void**>& Q1, TSQueue<void**>& Q2)
{
    // void** bufferPointer1 = allocateBuffer(engine, false, true);
    // std::vector<void*> gpuOutputBuffers(bufferPointer1, bufferPointer1 + config.batch_size_);
    while(i > 0)
    {
	    void** bufferPointer2 = Q1.pop();
	    ...
	    Q2.push(gpuOutputBuffers.data());
    }
    // for (size_t j = 0; j < gpuOutputBuffers.size(); ++j)  
    //     cudaFree(gpuOutputBuffers[j]);
    return;
}

void QDisplay(TSQueue<void**>& Q2)
{
    ...
    while(i > 0)
    {   
        void** bufferPointer = Q2.pop();
        // std::vector<void*> gpuOutputBuffers(bufferPointer, bufferPointer + config.batch_size_);
...
{   
    ...
    TSQueue<void**> Q1;
    TSQueue<void**> Q2;
    Q1.setLength(config.queue_max_length_);
    Q2.setLength(config.queue_max_length_);
    ...
    threads.emplace_back(std::thread(QCapture, std::ref(Q1)));
    threads.emplace_back(std::thread(QModel, std::ref(Q1), std::ref(Q2)));
    threads.emplace_back(std::thread(QDisplay, std::ref(Q2)));
