...
while i > 0:
    ...
    # H2D copy
    noisy_frame = np.ascontiguousarray(noisy_frame).ravel()
    if copied < frame_cnt:
        np.copyto(host_inputs[copied], noisy_frame)
        cuda.memcpy_htod_async(cuda_inputs[copied], host_inputs[copied], stream)
        copied += 1
        end = time.time()
        runtime = end - start
